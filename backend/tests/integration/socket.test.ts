...
// ── Ownership middleware: pass through ──
vi.mock('../../src/middleware/ownership.middleware.js', () => ({
  verifySessionOwnership: vi.fn((_req: unknown, _res: unknown, next: () => void) => next()),
  verifyRoomOwnership: vi.fn((_req: unknown, _res: unknown, next: () => void) => next()),
}));
